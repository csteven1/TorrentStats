...
jQuery.fn.dataTable.render.percentBar = function(pShape, cText, cBorder, cBar, cBack, vRound, bType) {
  ...
  bType = bType || 'ridge';
  //Bar templates
  var styleRule1 = 'max-width:100px;height:20px;margin:0 auto;';
  var styleRule2 = 'border:3px '+bType+' '+cBorder+';line-height:15px;font-size:13px;color:'+cText+';background:'+cBack+';position:relative;';
  var styleRule3 = 'height:16px;line-height:12px;text-align:center;background-color:'+cBar+';padding:auto 6px;';
  //Square is default, make template round if pShape == round
  if(pShape=='round'){
    styleRule2 += 'border-radius:5px;';
    styleRule3 += 'border-top-left-radius:1px;border-bottom-left-radius:1px;';
  }

  return function(d, type, row) {
    //Round to the given parameter vRound
	s = (d*100).toFixed(vRound);
